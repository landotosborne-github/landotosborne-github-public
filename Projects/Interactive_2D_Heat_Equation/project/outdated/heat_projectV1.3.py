...
class heat_system(object):
	def __init__(self, runtime , rate, l , top , right , bottom, left, bk_tp, ob_tp, bk_temp, ob_temp):
		...
		self.dt = 1 #UNITS seconds
		self.x_size = self.grid//100 #size in meters
		self.dx = 0.01#m UNITS: meters  dx is 1 cm
...
#Run a stability test for system given specified tp
	def tp_test(self, tp):
		check = (self.dx**2) / (4*self.dt)
		return tp <= check #tp must be less than 2.5e-5
...
#makes sure resulting array is not too large
	def size_test(self):
		max_npsize= 7.5e7
		return max_npsize > ((self.runtime//self.rate)*self.grid**2)
...
def doub_plot(gtot, x_size, grid):
	frames = len(gtot)
	gx = np.linspace(0, x_size*100 , grid)
	gy = np.linspace(0, x_size*100 , grid)
	...
	cba = f1.colorbar(k, ax=ax2)
	cba.set_label('Temperature (K)')
...
def anim_plot(gtot, grid, is_ob, runtime):
	frames=len(gtot)
	f = plt.figure(figsize = (15,10))
	ax1 = f.add_subplot(1,2,1)
	a = ax1.imshow(gtot[frames-1] , cmap='inferno', extent=[0,grid,0,grid])
	ax1.set_title('System Temperature over time')
	ax1.set_xlabel('X (cm)')
	ax1.set_ylabel('Y (cm)')
	cba = f.colorbar(a, ax=ax1)
	cba.set_label('Temperature (K)')
	
	ax2 = f.add_subplot(1,3,3)
	ax2.plot(np.linspace(0,runtime,frames),np.array([g[is_ob].mean() for g in gtot]))
	ax2.set_xlabel('Time (s)')
	ax2.set_ylabel('Average Object Temperature (K)')
	ax2.set_title('Avg. Object Temperature during Simulation')
...
def inputbox_resolution(stdscr,val):
	while True:
		...
		try:
			var = int(var)
			if var <= 0 or var > 1000 or var%100:
				raise ValueError('Input out of bounds')
			break
		except ValueError:
			curses.curs_set(0)
...
def input_settings(stdscr):
	...
	if op ==1:
		...
		choose_shape = menubox(stdscr, ['None','Circle','Square','Hexagon','Cross','Gear','6_star', '9_star','Paw','Stick_fig', 'Hello','Custom'])
		if choose_shape==1:
			pathtoshape='./shapes/project_none.png'
		elif choose_shape==2:
			...
			pathtoshape='./shapes/project_gear.png'
		elif choose_shape==7:
			pathtoshape='./shapes/project_6star.png'
		elif choose_shape==8:
			pathtoshape='./shapes/project_9star.png'
		elif choose_shape==9:
			pathtoshape='./shapes/project_paw.png'
		elif choose_shape==10:
			pathtoshape='./shapes/project_stickfig.png'
		elif choose_shape==11:
			pathtoshape='./shapes/project_hello.png'
		elif choose_shape==12:
			pathtoshape='./shapes/project_custom.png'
		...
		stdscr.addstr(1,0,'Units: seconds    Recommended: ~1000 s')
		stdscr.addstr(2,0,'Warning: Large inputs lead to longer load times!')
		runtime = inputbox_intreal(stdscr, 'runtime')

		stdscr.clear()
		stdscr.addstr(0,0,'Format: positive integer (less than 1,000 and multiple of 100)')
		stdscr.addstr(1,0,'Units: centimeters    Recommended: 100 cm')
		stdscr.addstr(2,0,'Warning: Large inputs lead to longer load times!')
		l = inputbox_resolution(stdscr, 'Size')
		
		stdscr.clear()
		stdscr.addstr(0,0,'Format: positive integer factor of runtime')
		stdscr.addstr(1,0,'Units: frames/s    Recommended: ~10 fps')
		...
		stdscr.addstr(1,0,'Units: Kelvin    Recommended:  >0 K')
		stdscr.addstr(2,0,'')
		...
		stdscr.addstr(1,0,'Units: m^2/s    Recommended: <2.5e-5 m^2/s')
		stdscr.addstr(2,0,'Warning: system stability check!')
		stdscr.addstr(10,0,'Thermal diffusivity of Common Materials when T=~300K:\nIron : 2.3e-5 m^2/s\nAir : 1.9e-5 m^2/s\nStainless Steel : 4.2e-6 m^2/s\nWater : 1.43e-7 m^2/s\nHuman : 1.23e-7 m^2/s\nWood : 8.2e-8 m^2/s')
		bk_tp = inputbox_float(stdscr, 'background thermal diffusivity')
		...
		stdscr.addstr(10,0,'Thermal diffusivity of Common Materials when T=~300K:\nIron : 2.3e-5 m^2/s\nAir : 1.9e-5 m^2/s\nStainless Steel : 4.2e-6 m^2/s\nWater : 1.43e-7 m^2/s\nHuman : 1.23e-7 m^2/s\nWood : 8.2e-8 m^2/s')
		ob_tp = inputbox_float(stdscr, 'object thermal diffusivity')
...
def check_inputs(stdscr,pathtoshape, runtime, l, rate, top, right, bottom, left, bk_tp, ob_tp, bk_temp, ob_temp):
	...
	stdscr.addstr(3,50,'Recommended: ~1000 s')
	
	s2='Grid Length : ' + str(l)
	stdscr.addstr(4,0,s2)
	stdscr.addstr(4,50,'Recommended: ~100 cm')
	
	s3='Framerate : ' + str(rate)
	stdscr.addstr(5,0,s3)
	stdscr.addstr(5,50,'Recommended: 5-10 fps')
	
	s4='Top : ' + str(top)
	stdscr.addstr(6,0,s4)
	stdscr.addstr(6,50,'Recommended: >0 K')
	
	s5='Right : ' + str(right)
	stdscr.addstr(7,0,s5)
	stdscr.addstr(7,50,'Recommended: >0 K')
	
	s6='Bottom : ' + str(bottom)
	stdscr.addstr(8,0,s6)
	stdscr.addstr(8,50,'Recommended: >0 K')
	
	s7='Left : ' + str(left)
	stdscr.addstr(9,0,s7)
	stdscr.addstr(9,50,'Recommended: >0 K')
	
	s8='Background thermal diffusivity : ' + str(bk_tp)
	stdscr.addstr(10,0,s8)
	stdscr.addstr(10,50,'Recommended: <2.5e-5 m^2/s')
	
	s9='Object thermal diffusivity : ' + str(ob_tp)
	stdscr.addstr(11,0,s9)
	stdscr.addstr(11,50,'Recommended: <2.5e-5 m^2/s')
	
	s10='Background temp : ' + str(bk_temp)
	stdscr.addstr(12,0,s10)
	stdscr.addstr(12,50,'Recommended: >0 K')
	
	s11='Object temp : ' + str(ob_temp)
	stdscr.addstr(13,0,s11)
	stdscr.addstr(13,50,'Recommended: >0 K')
...
def input_tests(stdscr, heat_system):
	...
	if heat_system.tp_test(heat_system.bk_tp):
		...
		stdscr.addstr(place,0,'PASS [1/5]: Background Thermal diffusivity passed stability test')
		stdscr.attroff(curses.color_pair(2))
		stdscr.refresh()
		place+=1
	else:
		stdscr.attron(curses.color_pair(3))
		stdscr.addstr(place,0,'FAIL [1/5]: Background Thermal diffusivity failed stability test! ')
	...
	if heat_system.tp_test(heat_system.ob_tp):
		stdscr.attron(curses.color_pair(2))
		stdscr.addstr(place,0,'PASS [2/5]: Object Thermal diffusivity passed stability test')
		stdscr.attroff(curses.color_pair(2))
		stdscr.refresh()
		place+=1
		pass
		...
		stdscr.attron(curses.color_pair(3))
		stdscr.addstr(place,0,'FAIL [2/5]: Object Thermal diffusivity failed stability test')
		stdscr.attroff(curses.color_pair(3))
		stdscr.refresh()
		check+=1
		place+=1
...
def check_inputs_step(stdscr,pathtoshape, step_size, l, top, right, bottom, left, bk_tp, ob_tp, bk_temp, ob_temp):
	...
	stdscr.addstr(3,0,s1)
	stdscr.addstr(3,50,'Recommended: 1-1000 s')
	...
	stdscr.addstr(5,50,'Recommended: >0 K')
	...
	stdscr.addstr(9,50,'Recommended: <2.5e-5 m^2/s')
	...
	stdscr.addstr(11,50,'Recommended: >0 K')
...
def display1(stdscr):
	...
	if yesnobox(stdscr):
		...
		if input_tests(stdscr, heat_sys):
			...
			stdscr.addstr(1,0,'Press <enter> to continue')
			stdscr.refresh()
			ani = anim_plot(gtot, heat_sys.grid, heat_sys.is_ob, heat_sys.runtime)
			plt.show()
			contin(stdscr)
		else:
			stdscr.clear()
...
def display3(stdscr):
	...
	if yesnobox(stdscr):
		...
		if input_tests(stdscr, heat_sys):
			...
			if b == 1:
				gtot[0, 1:-1, 1:-1] = gtot[0,1:-1,1:-1] + np.random.uniform(-1,1,size = (l-2,l-2))*heat_sys.ob_temp
				pass
			elif b == 2:
				gtot[0, 1:-1, 1:-1] = gtot[0,1:-1,1:-1] + np.random.uniform(0,1,size = (l-2,l-2))*heat_sys.ob_temp
				pass
			elif b == 3:
...
def help_display(stdscr):
	...
	choose = menubox(stdscr, ['Quick Guide','The Math behind the Simulation','Why do my settings keep failing input tests?','What is the stability test?','Why do I get stuck when the plots open?','What is "invalid command name "2873334728_on_timer""?', 'How to use Custom shape?'])
	...
	if choose==1:#quick guide
		...
		stdscr.addstr(1,0,'Press <enter> to reset')
		stdscr.addstr(2,0,'Some settings need to be in a certain format or factors of each other for some modes to function properly. Try reading the recommended values or using the Simple/Default simulation mode.\nSome of the most commmon settings that need specific formatting are the Runtime and Framerate, the framerate must be a factor of the runtime and the runtime cannot be too large, the Resolution, which must be a factor of 100, and the thermal diffusivity, which must be less than 2.5e-5 m^2/s and pass the system stability tests')
		stdscr.refresh()
		stdscr.getch()
		...
		stdscr.addstr(1,0,'Press <enter> to reset')
		stdscr.addstr(2,0,'The stability test is a test performed on the user inputted thermal diffusivity which ensures that the error involved in the finite difference calculations is stable and will remain constant or shrink as the simulation runs. It uses Von-Neumann Stability analysis and for our purposes it means the thermal diffusivity times the time differential over the space differential squared must be less than 1/4\n-- a*(dt/dx**2) <= 0.25 --\nFor more information on this visit\nhttps://en.wikipedia.org/wiki/Von_Neumann_stability_analysis')
		stdscr.refresh()
		stdscr.getch()
	...
	elif choose == 7:#Custom
		stdscr.clear()
		stdscr.addstr(0,0,'How to use Custom shape?', curses.A_BOLD)
		stdscr.addstr(1,0,'Press <enter> to reset')
		stdscr.addstr(2,0,'To enter a custom shape you can overwrite the file project_custom.png with a new .png image.\nThe custom .png image must be 100x100 pixels.\nAny pixels with an average RGB brightness below 100 will be defined as the objects pixels.\nBlack and white pictures work best, but the main requirement is that the image be 100x100 pixels.\nHave Fun!')
		stdscr.refresh()
		stdscr.getch()
...
def main(stdscr):
	...
	welcome= "Welcome to the 2D Heat Equation Simulator! v1.3"
